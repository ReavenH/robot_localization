    ...
    try:
        ...
        if myRobot.currentAction == 'S':
            ...
            for i in range(5): # default 5
                ...
                while abs(myRobot.bottomLineCentroid[0]) > 45:
                    if myRobot.bottomLineCentroid[0] > 55:
                        myRobot.triangularwalk(-90, np.ceil(np.abs(myRobot.bottomLineCentroid[1]) / 120 * (15 - 5) + 5), continuous=False)
                    elif myRobot.bottomLineCentroid[0] < -55:
                        myRobot.triangularwalk(90, np.ceil(np.abs(myRobot.bottomLineCentroid[1]) / 120 * (15 - 5) + 5), continuous=False)
                myRobot.freeturn(20) # default zh_planarWalk1.py
                # time.sleep(2.5)
            
            # adjust yaw before forward.
            time.sleep(1.2)
            myRobot.getPoseFromCircles(minCircles=5, rotAid=True)
            while myRobot.lostVision in [-2, 2, 0]:
