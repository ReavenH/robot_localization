...
while myRobot.cap.isOpened:
    try:
        ...
        if myRobot.currentAction == 'S':
            ...
            myRobot.freeturn(-25)
            time.sleep(1.3)
            # n = 12 if myRobot.carryingBrick else 10
            for i in range(12): # default 5
                # myRobot.waitGlobalStep()
            ...
            '''
            '''
                print("LostVision: {}, lostVisionCrossing: {},{}".format(myRobot.lostVision, myRobot.lostVisionCrossingX, myRobot.lostVisionCrossingY))
                if len(myRobot.bottomLineYaw) == 2:
                    if myRobot.bottomLineCentroidCrossing[1] < -25:
                        myRobot.triangularwalk(0, 12, continuous = False)
                        myRobot.waitGlobalStep()
                    if myRobot.bottomLineCentroidCrossing[1] > 25:
                        myRobot.triangularwalk(0, -12, continuous = False)
                        myRobot.waitGlobalStep()
                    if myRobot.bottomLineCentroidCrossing[0] < -55:
                        myRobot.triangularwalk(90, 12, continuous = False)
                        myRobot.waitGlobalStep()
                    if myRobot.bottomLineCentroidCrossing[0] > 55:
                        myRobot.triangularwalk(-90, 12, continuous = False)
                    ...
                    dir = myRobot.lostVisionCrossingX * myRobot.lostVisionCrossingY * 45
                    myRobot.triangularwalk(dir, 22, continuous = False)
                    myRobot.waitGlobalStep()
                '''
                # time.sleep(2.5)
            '''
            ...
            myRobot.singleServoCtrl(0, myRobot.servoCriticalAngles["linkageFlat"] - 100, 1/10)
            time.sleep(1)
        ...
        #sortclimb
            ...
            print("Ending 2nd Phase of Climbing")
